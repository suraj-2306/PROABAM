function [p_cout, p_carry, p_sum] = comp_42_exa(array_in)
    a = array_in(1);
    b = array_in(2);
    c = array_in(3);
    d = array_in(4);
    % the sum  array with 15 as the approximation
    % minterm_sum = [1, 2, 4, 7, 8, 11, 13, 14,15];
    % the carry sum with 15 as the approximation
    % minterm_carry = [3, 5, 6, 7, 9, 10, 11, 12, 13, 14, 15];

    % the sum  array
    minterm_sum = [1, 2, 4, 7, 8, 11, 13, 14];
    % the carry sum
    minterm_carry = [3, 5, 6, 7, 9, 10, 11, 12, 13, 14, 15];
    minterm_cout = [15];
    p_carry = 0;
    ...
    p_cout = minterm_bit4(a, b, c, d, minterm_cout);
    % p_sum = ()
